...
if __name__ == "__main__":
    ...
    try:
        # 解锁飞控
            # 配置 AUX OUT 通道 1（通常为通道 9）为手动模式
            aux_channel = 9  # AUX OUT 通道 1 对应的编号
            configure_servo_function(master, aux_channel, 0)
            ...
            print(f"Controlling servo on AUX OUT channel {aux_channel}...")
            for pwm in range(500,2000, 100):
                set_servo_pwm(master, aux_channel, pwm)
                time.sleep(0.5)  # 调整延时控制转速

            # 返回中位并锁定
            set_servo_pwm(master, aux_channel, 1500)
            time.sleep(1)
